 ...
 * 
 * Something that can happen during the course of the game. A pop-up is shown to
 * the user which they can react to by
...
public class GameEvent {
    ...
    private Event mapEvent; // Corresponding MapObject for placement of the event icon -> used in
                            // EventHandler
    private boolean isActive; // True/false if active/inactive
    private LocalDateTime eventStartedAt;
    private float responseEffect; // the effect on the score the picked event response caused - this will be
    ...
    /**
     * This function checks all the events against the current game time, and
     * removes them.
     * Only one
     * 
